...
/*  Returns the waveformatex for known formats.
    Arguments:pDataFormat - data format.
    Return Value: waveformatex in DataFormat. NULL for unknown data formats. */
...
/*   Default basic support handler. Basic processing depends on the size of data.
     ...
     is filled.
     Arguments:  Flags - Support flags. */
NTSTATUS PropertyHandler_BasicSupport ( IN PPCPROPERTY_REQUEST PropertyRequest,
...
NTSTATUS ValidatePropertyParams ( IN PPCPROPERTY_REQUEST PropertyRequest, 
                                  IN ULONG cbSize, IN ULONG cbInstanceSize /* = 0  */)
{
